...
def test_save_imap_persists_per_user_and_masks_password(tmp_path, monkeypatch):
    """IMAP config is saved per-profile and the password is never echoed
    ...
    with TestClient(app) as client:
        ...
        stored = asyncio.get_event_loop().run_until_complete(read())
    ...
    # Host is rendered back; the secret never is.
    assert "imap.mailbox.org" in page.text
    assert "topsecretpw" not in page.text
